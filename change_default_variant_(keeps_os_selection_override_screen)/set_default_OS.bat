...
for /f "tokens=2" %%i in ('bcdedit /enum /v ^| findstr /i "default"') do @echo %%i > "%~dp0default_OS.txt"
...
for /f "tokens=2" %%i in ('bcdedit /enum "{current}" /v ^| findstr /i "identifier"') do @echo %%i > "%~dp0old_current_OS.txt"

@REM enable the revert process on next boot
echo true > "%~dp0engaged.txt"

@REM set default OS to the currently booted one
for /f "usebackq tokens=*" %%a in (`bcdedit /default "{current}" 2^>^&1`) do echo [	%~n0%~x0	]	[	%time%	] %%a >> "%~dp0log.txt" 2>&1
